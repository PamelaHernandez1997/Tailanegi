...
let appState = {
    projectionData: null,          // Datos descargados del backend
    activeRegionKey: 'Total',      // 'Total' (Nacional) o llave de REGIONS
    activeYear: 2018,              // Año seleccionado
    chartInstance: null,           // Instancia del gráfico Chart.js
    selectedRegionKeyMap: 'Northern' // Región por defecto en la pestaña del Mapa
};
...
// Base de datos de hitos narrativos por región y año
const NARRATIVE_DATABASE = {
    'Total': {
        ...
        2035: "Pico histórico y estancamiento. La población nacional alcanza su cúspide histórica cerca de los 72.2M de habitantes. Las defunciones superan a los nacimientos a nivel nacional, la migración es el único factor de compensación.",
    ...
    'Southern': {
        ...
        2025: "Desarrollo estable. El turismo internacional y la economía costera sostienen un crecimiento poblacional moderado de 9.36M hab.",
...
async function initApp() {
    ...
    // Descargar datos iniciales del backend
    ...
    // Configurar solver por defecto con la región seleccionada
...
async function fetchProjectionData() {
    try {
        ...
        alert('No se pudo conectar con el servidor backend de Python (app.py). La aplicación funcionará con datos simulados locales.');
        loadMockData(); // Carga de respaldo si falla la API
    }
}

// Respaldo de datos simulados locales por seguridad
function loadMockData() {
    ...
    appState.projectionData = {
        ...
        regions: {
            'Northern': {
                name_es: 'Región Norte', p0: 11500000, k: -0.0035, description: 'Región montañosa con alto índice de envejecimiento.', trend: 'Decrecimiento continuo.',
                projection: years.map(y => Math.round(11500000 * Math.exp(-0.0035 * (y - 2018))))
            },
            'Northeastern': {
                name_es: 'Región Nordeste (Isan)', p0: 21500000, k: -0.0012, description: 'La región más poblada, estancamiento y transición a declive.', trend: 'Estancamiento y leve transición.',
                projection: years.map(y => Math.round(21500000 * Math.exp(-0.0012 * (y - 2018))))
            },
            'Central': {
                name_es: 'Región Centro', p0: 21200000, k: 0.0038, description: 'Corazón financiero e industrial, incluye Bangkok.', trend: 'Crecimiento estable.',
                projection: years.map(y => Math.round(21200000 * Math.exp(0.0038 * (y - 2018))))
            },
            'Eastern': {
                name_es: 'Región Este', p0: 5600000, k: 0.0075, description: 'Eje industrial y tecnológico (EEC).', trend: 'Crecimiento dinámico.',
                projection: years.map(y => Math.round(5600000 * Math.exp(0.0075 * (y - 2018))))
            },
            'Western': {
                name_es: 'Región Oeste', p0: 2376079, k: -0.0022, description: 'Región fronteriza y predominantemente forestal.', trend: 'Decrecimiento moderado.',
                projection: years.map(y => Math.round(2376079 * Math.exp(-0.0022 * (y - 2018))))
            },
            'Southern': {
                name_es: 'Región Sur', p0: 9200000, k: 0.0025, description: 'Región costera con mayores tasas de fecundidad.', trend: 'Crecimiento lento.',
...
function setupTabNavigation() {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            // Refrescar el gráfico si se cambia al dashboard
...
// Cambiar de pestaña programáticamente
...
function setupTimelineSlider() {
    ...
    // Escuchar el cambio en tiempo real (input)
...
// Sincronizar el selector del solver matemático con la región activa
...
// ==========================================================================
// 7. ACTUALIZACIÓN DEL DASHBOARD E INDICADORES (CON ALERTAS ROJAS)
// ==========================================================================
function updateDashboardUI() {
    ...
    // Obtener datos según la región activa
    if (appState.activeRegionKey === 'Total') {
        ...
        trend = "Crecimiento nacional lento con marcadas asimetrías regionales.";
        isDeclining = false; // El total nacional sigue creciendo levemente neto
    } else {
        ...
        trend = reg.trend;
        isDeclining = k < 0; // Alerta si la tasa es negativa
    }
    ...
    // Estado Demográfico y Alertas Rojas de Baja Población
    ...
    // Elementos a los que se les aplica la clase roja brillante
    ...
    if (isDeclining) {
        // Modificar tarjetas a Alerta Crítica (Fondo Rojo Brillante e Indicadores)
        ...
        // Mostrar Banner de Alerta Crítica superior
        alertBanner.classList.remove('hidden');
    } else {
        // Restaurar estado normal (Gris/Blanco)
        ...
        // Ocultar Banner de Alerta Crítica superior
        alertBanner.classList.add('hidden');
    }

    // Actualizar Narrativa del Hito de Storytelling
    updateNarrativeUI(appState.activeRegionKey, appState.activeYear);
}

// Actualiza el texto descriptivo del storytelling
function updateNarrativeUI(regionKey, year) {
    ...
    // Buscar hito de año específico o encontrar el hito más cercano anterior
...
function initProjectionChart() {
    ...
    // Obtener proyección activa inicial (Total)
    ...
    const datasets = [
        ...
        // Dataset de resaltado del año seleccionado (un único punto en la gráfica)
        {
    ...
    appState.chartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                legend: {
                    display: false // Ocultamos leyenda ya que se explica en la cabecera
                },
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            if (context.datasetIndex === 1) return 'Punto Activo';
...
// Actualizar la curva cuando cambia de región activa
function updateProjectionChart() {
    ...
    // Cambiar colores si es decreciente o creciente
    const primaryColor = isDeclining ? '#EF4444' : '#0056B3';
    const ctx = document.getElementById('projectionChart').getContext('2d');
    
    // Generar nuevo gradiente correspondiente
    const gradient = ctx.createLinearGradient(0, 0, 0, 350);
    if (isDeclining) {
    ...
    // Actualizar punto de realce
    updateChartHighlight();
}

// Actualizar el realce del punto correspondiente al año seleccionado
function updateChartHighlight() {
    ...
    // Rellenar con nulls excepto el año activo
    ...
    appState.chartInstance.update('none'); // Update sin animación para evitar lag
}
...
function setupMapInteractions() {
    ...
    paths.forEach(path => {
        ...
        // Hover: Actualizar temporalmente el panel lateral
        path.addEventListener('mouseenter', () => {
            updateMapSidebar(regionKey);
        });

        // Leave: Regresar a los datos de la región seleccionada del mapa
        path.addEventListener('mouseleave', () => {
            updateMapSidebar(appState.selectedRegionKeyMap);
        });

        // Click: Hacer que esta región sea la seleccionada permanentemente
        path.addEventListener('click', () => {
            selectRegionOnMap(regionKey);
            
            // Sincronizar también con la pestaña del dashboard
            appState.activeRegionKey = regionKey;
            
            // Actualizar botones de dashboard
...
// Inicializar y dar colores dinámicos a las regiones en el mapa
function updateMapUI() {
    ...
    paths.forEach(path => {
        ...
        // Agregar clase si la región tiene decrecimiento (alertas en rojo)
        if (reg && reg.k < 0) {
    ...
    // Cargar los datos de la primera región por defecto (Norte)
    updateMapSidebar(appState.selectedRegionKeyMap);
}

// Seleccionar visualmente una región en el mapa
...
// Actualizar los datos del panel lateral del mapa interactivo
function updateMapSidebar(regionKey) {
    if (!appState.projectionData) return;

    // El mapa no tiene opción nacional, si se pasa Total por algún motivo, usar Norte
    if (regionKey === 'Total') regionKey = 'Northern';
    ...
    const sidebarCard = document.getElementById('map-sidebar-card');
    const sidebarContainer = document.getElementById('map-details-panel');

    // Cambiar colores si la tasa es negativa (Alertas en Rojo)
    if (reg.k < 0) {
...
// ==========================================================================
// 10. SOLVER INTERACTIVO (MALTHUS SOLVER FORM)
// ==========================================================================
function setupSolverForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            console.error('Error solving population:', error);
            alert('Ocurrió un error al contactar al solver del backend.');
        } finally {
            // Ocultar cargando
...
// Desplegar los resultados de coincidencia del solver exponencial en pantalla
function displaySolverResult(data) {
    ...
    if (data.success) {
        ...
        // Mostrar error (alerta en rojo) y ocultar éxito
    ...
    // Scroll suave hasta el cuadro de resultados
    resultBox.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}

// ==========================================================================
// 11. BUSCADOR GLOBAL INTELIGENTE (BÚSQUEDAS E INTERACCIONES)
// ==========================================================================
function setupSearch() {
    ...
    // Escuchar cambios de escritura para mostrar botón borrar
...
// Ejecución de la búsqueda según tipo de patrón
function performSearch(query) {
    ...
    if (yearMatch) {
        ...
        // Cambiar a la pestaña del dashboard
        switchTab('tab-dashboard');
        return;
    }

    // 2. PATRÓN DE REGIONES
    const regionsMap = {
    ...
    if (matchedRegion) {
        // Aplicar cambio de región activa
        appState.activeRegionKey = matchedRegion;
        
        // Sincronizar botones de dashboard
        const dashboardButtons = document.querySelectorAll('.region-filter-btn');
        dashboardButtons.forEach(b => {
            b.classList.remove('active');
        ...
        // Sincronizar mapa si no es nacional
        ...
        // Redirigir a pestaña dashboard
        switchTab('tab-dashboard');
        return;
    }

    // 3. PATRÓN DE POBLACIÓN (Ej: "72M" o "72000000" o "10.5M")
    // Detectar números con M o millones
    let popValue = 0;
    ...
    if (mMatch) {
        ...
        // Considerar población si el valor es mayor a 1,000,000
        if (val >= 100000) {
            popValue = val;
        }
    }
    ...
    // Si nada coincide, mostrar mensaje de error de búsqueda
